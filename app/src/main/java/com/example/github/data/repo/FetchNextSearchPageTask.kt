...
import com.example.github.data.api.*
...
class FetchNextSearchPageTask constructor(
    ...
    private val githubService: GitService,
    ...
    override fun run() {
        val current = db.gitRepositoryDao().findSearchResult(query)
        if (current == null) {
        ...
        val newValue = try {
            val response = githubService.searchGitRepositories(query, "stars",nextPage, 100)
            when (val apiResponse = response.value) {
                is ApiSuccessResponse -> {
                    // we merge all repo ids into 1 list so that it is easier to fetch the
                    // result list.
                    val ids = arrayListOf<Int>()
                    ids.addAll(current.gitRepositoryIds)

                    ids.addAll(apiResponse.body.items.map { it.id })
                    val merged = GitRepositoriesSearchResult(
                        query, ids,
                        apiResponse.body.total, apiResponse.nextPage
                    )
                    db.runInTransaction {
                        db.gitRepositoryDao().insertSearchResult(merged)
                        db.gitRepositoryDao().insertManyGitRepos(apiResponse.body.items)
                    }
                ...
                null -> Resource.error("RESPONSE == null", false)
            }
